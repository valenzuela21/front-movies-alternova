...
const  Register = () =>{
    ...
    const formik =  useFormik({
        ...
        validationSchema: yup.object({
            namefull: yup.string().required("The input name is required"),
            email: yup.string().email('The input email not validate').required("The input email is required"),
            password: yup.string().required("The input password is required"),
        }),
        onSubmit: async (values) => {
            try{
                await  clientAxios.post('/api/auth/register', values)
                formik.resetForm();
                setState(true);
            }catch (error){
                setState(false);
                            ...
                            <form onSubmit={formik.handleSubmit} className="space-y-4 md:space-y-6" >
                                <div>
                                    ...
                                    <input type="text" name="namefull" id="namefull"
                                           ...
                                           placeholder="Fernandez Pardo"
                                           onChange={formik.handleChange}
                                           onBlur={formik.handleBlur}
                                           value={formik.values.namefull}
                                    />
                                </div>
                                {formik.touched.namefull && formik.errors.namefull ? (
                                    <div
                                ...
                                <p className="text-sm font-light text-gray-500 dark:text-gray-400">
                                    Ya tienes una cuenta? <Link href="/login" className="font-medium text-primary-600 hover:underline dark:text-primary-500">Ingresar </Link>
                                </p>
                            </form>
                        </div>
                    </div>
